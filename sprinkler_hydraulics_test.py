...
PFA = [0.   , 0.613, 0.737, 0.566, 0.612, 0.568, 0.597, 0.656, 0.517, 0.731]
IDR = [0.   , 0.377, 0.312, 0.328, 0.314, 0.326, 0.402, 0.406, 0.63 , 0.934]

# def Sprinkler(PFA,IDR,plot=False):
...
for ename in wn.pipe_name_list:
    ...
    if e.ds == 2:
        theta = 0.5             # degrees
        leak_area = np.sqrt(2*theta)*e.diameter/2.
        print(ename+' ds2')
    elif e.ds == 3:
        theta = 0.5
        leak_area = np.sqrt(2*theta)*e.diameter
        print(ename+' ds3')
    if e.ds>=2:
